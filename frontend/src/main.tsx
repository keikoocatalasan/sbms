...
import { CustomerDialog, CustomerProfileDialog, InvoiceDetailDialog, InvoiceDialog, NotificationDialog, PaymentAllocationDialog, PaymentDialog, PlanDialog, PlanPriceDialog, SubscriptionCommandDialog, SubscriptionDialog } from './dialogs'
...
function Shell({ children }: { children: React.ReactNode }) {
  ...
  return <div className={`shell ${collapsed ? 'collapsed' : ''}`}>
    ...
    <main><header><button type="button" className="mobile-menu" aria-label="Open navigation" onClick={() => setCollapsed(true)}><Menu/></button><div className="header-spacer"/><span className="date">{shortDate(new Date().toISOString())}</span><button type="button" className="notification-bell icon-button" aria-label={`${unread} unread notifications`} onClick={() => navigate('/notifications')}><Bell size={21}/>{unread > 0 && <b>{unread}</b>}</button><div className="profile-wrap"><button type="button" className="profile-button" aria-label="Open profile menu" aria-expanded={profileOpen} onClick={() => setProfileOpen(value => !value)}><span className="profile-photo">{user.name[0]}</span><span><b>{user.name}</b><small>{can('subscription:admin') ? 'Administrator' : user.scopes.includes('subscription:billing') ? 'Billing specialist' : 'Read-only member'}</small></span></button>{profileOpen && <div className="profile-menu"><p>{user.email}</p><button type="button" onClick={() => { setProfileOpen(false); signOut() }}>Sign out</button></div>}</div></header>{children}</main>
  </div>
}
...
function SettingsPage() {
  ...
  async function save(event: FormEvent) { event.preventDefault(); if (!form) return; setBusy(true); try { const { default_currency, timezone, invoice_due_days, grace_period_days, allow_partial_payments, auto_renew_default, auto_generate_invoices, enable_in_app_notifications, invoice_prefix, payment_prefix, subscription_prefix, customer_prefix } = form; await api.patch('/settings', { default_currency, timezone, invoice_due_days, grace_period_days, allow_partial_payments, auto_renew_default, auto_generate_invoices, enable_in_app_notifications, invoice_prefix, payment_prefix, subscription_prefix, customer_prefix }); await refresh(); done('Settings saved.'); } catch (caught) { fail(caught, 'Unable to save settings.') } finally { setBusy(false) } }
