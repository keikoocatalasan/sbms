...
function LoginPage() {
  const navigate = useNavigate()
  const [email, setEmail] = useState('admin@argo.demo')
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await api.post<Envelope<{ access_token: string; user: AuthUser }>>('/auth/login', { email, password })
      saveSession(response.data.data.access_token, response.data.data.user)
      navigate('/dashboard', { replace: true })
      window.location.reload()
    } catch (caught) { setError(apiMessage(caught, 'Unable to sign in.')) } finally { setBusy(false) }
  }
  return <div className="login"><form onSubmit={submit}><span className="brand-logo"><LayoutDashboard size={25}/></span><h1>Welcome to Argo</h1><p>Sign in to Subscription Management</p><label>Email<input value={email} onChange={event => setEmail(event.target.value)} type="email" required autoComplete="username"/></label><label>Password<input value={password} onChange={event => setPassword(event.target.value)} type="password" required minLength={8} autoComplete="current-password"/></label>{error && <p className="error" role="alert">{error}</p>}<button className="button primary" disabled={busy}>{busy ? 'Signing in…' : 'Sign in'}</button><small>Administrator: admin@argo.demo</small><small>Billing: billing@argo.demo</small><small>Password: DemoPass123!</small></form></div>
}
